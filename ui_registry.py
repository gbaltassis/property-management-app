...
def show():
        ...
        if sub_prop == "📋 Ιστορικό / Λίστα":
            ...
            if properties_df.empty: st.warning("Δεν υπάρχουν ακίνητα.")
                ...
                if selected_edit_id:
                    ...
                    if st.button("🗑️ Οριστική Διαγραφή Ακινήτου", type="primary"):
                        try:
    ...
    # =====================================================================
    # 2. ΕΝΟΙΚΙΑΣΤΕΣ (Υπο-μενού)
    # =====================================================================
    with main_tab_tenant:
        sub_tenant = st.radio("Ενέργεια:", ["📋 Ιστορικό / Λίστα", "➕ Νέος Ενοικιαστής", "✏️ Επεξεργασία"], horizontal=True, key="sub_tenant", label_visibility="collapsed")
        st.write("")
        
        if sub_tenant == "📋 Ιστορικό / Λίστα":
        ...
        elif sub_tenant == "➕ Νέος Ενοικιαστής":
            ...
            if tenants_df.empty: st.warning("Δεν υπάρχουν ενοικιαστές.")
                ...
                if selected_t_edit:
                    ...
                    if st.button("🗑️ Οριστική Διαγραφή Ενοικιαστή", type="primary"):
                        try:
    ...
    # =====================================================================
    # 3. ΑΣΦΑΛΙΣΤΗΡΙΑ (Υπο-μενού)
    # =====================================================================
    with main_tab_ins:
        sub_ins = st.radio("Ενέργεια:", ["📋 Ιστορικό / Λίστα", "➕ Νέο Ασφαλιστήριο", "✏️ Επεξεργασία"], horizontal=True, key="sub_ins", label_visibility="collapsed")
        st.write("")
        
        if sub_ins == "📋 Ιστορικό / Λίστα":
            ...
            if insurances_df.empty: st.warning("Δεν υπάρχουν ασφαλιστήρια.")
                ...
                for _, r in insurances_df.iterrows():
                    ...
                    p_val_raw = str(r.get("Property_ID", ""))
                    p_charact = "-"
                    if not properties_df.empty and p_val_raw in properties_df["Property_ID"].values:
                        p_charact = properties_df[properties_df["Property_ID"] == p_val_raw].iloc[0].get("Χαρακτηριστικό", "-")
                ...
                if selected_i_edit:
                    ...
                    if st.button("🗑️ Οριστική Διαγραφή Ασφαλιστηρίου", type="primary", use_container_width=True):
                        try:
